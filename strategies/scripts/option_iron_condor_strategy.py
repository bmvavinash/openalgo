#!/usr/bin/env python
"""
Option Iron Condor Strategy
Sell OTM Call + Sell OTM Put, Buy further OTM Call + Buy further OTM Put
Neutral strategy with limited risk and limited profit
...
sell_otm = int(os.getenv('SELL_OTM', '5'))  # OTM level for short strikes
buy_otm = int(os.getenv('BUY_OTM', '10'))   # OTM level for long strikes (wings)
...
def place_iron_condor():
    """Place an Iron Condor using optionsmultiorder API"""
    try:
        print(f"[{datetime.now()}] Placing {strategy_name} for {underlying}")
        print(f"Short strikes: OTM{sell_otm}, Long strikes (wings): OTM{buy_otm}")
        
        # Iron Condor legs:
        # 1. Buy OTM Call (upper wing)
        # 2. Buy OTM Put (lower wing)
        # 3. Sell OTM Call (upper short)
        # 4. Sell OTM Put (lower short)
        # Note: BUY legs execute first, then SELL legs (for margin efficiency)
        ...
        print(f"[{datetime.now()}] Error placing Iron Condor: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    print(f"Short OTM: {sell_otm}, Long OTM: {buy_otm}")
    print("-" * 70)
    ...
    try:
        while True:
            ...
            # - Stop loss (e.g., 200% of premium received)
            # - Time-based exit (e.g., close 1 day before expiry)
    except KeyboardInterrupt:
        print(f"\n[{datetime.now()}] Strategy stopped by user")
        # Note: Closing Iron Condor requires reversing all 4 legs
        # This would need to be implemented based on position tracking
