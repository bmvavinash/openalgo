#!/usr/bin/env python
"""
Option Bull Call Spread Strategy
Buy ITM/ATM Call + Sell OTM Call
...
buy_offset = os.getenv('BUY_OFFSET', 'ITM2')  # ITM/ATM for long call
sell_otm = int(os.getenv('SELL_OTM', '5'))    # OTM level for short call
...
def place_bull_call_spread():
    """Place a Bull Call Spread using optionsmultiorder API"""
    try:
        print(f"[{datetime.now()}] Placing {strategy_name} for {underlying}")
        print(f"Buy: {buy_offset} Call, Sell: OTM{sell_otm} Call")
        
        # Bull Call Spread legs:
        # 1. Buy ITM/ATM Call (long leg)
        # 2. Sell OTM Call (short leg)
        ...
        print(f"[{datetime.now()}] Error placing Bull Call Spread: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    # Place Bull Call Spread
    place_bull_call_spread()
    
    try:
        while True:
            time.sleep(60)  # Check every minute
            # Add exit logic here:
            # - Profit target (e.g., 80% of max profit)
            # - Stop loss (e.g., 50% of premium paid)
