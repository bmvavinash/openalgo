...
def tradebook():
    ...
    if not success:
        ...
        if status_code == 404:
            return "Failed to import broker module", 500
        # In paper trading mode, return empty tradebook instead of redirecting
        if get_analyze_mode():
            return render_template('tradebook.html', tradebook_data=[])
        return redirect(url_for('auth.logout'))
